                            ...
                            <div class="col-md-6">
                                <table class="table table-borderless">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            {{ $vehicleDetail->tanggal_berlaku
                                                ? $vehicleDetail->tanggal_berlaku->format('d/m/Y')
                                                : '-' }}
                                        </td>
                                    </tr>
                                    <tr>
...
<tr>
    ...
    <td>
        @php
            $badgeClass = match($m->jenis_servis) {
                'Pengisian BBM' => 'bg-info',
                'Service Rutin' => 'bg-success',
                'Perbaikan'     => 'bg-warning',
                'Penggantian'   => 'bg-primary',
                default         => 'bg-danger',
            };
        @endphp
    ...
    <td>
        @if($m->jenis_servis == 'Pengisian BBM')
            {{ $m->jenis_bbm }} - {{ $m->jumlah_liter }}L
            @ Rp{{ number_format($m->harga_per_liter, 0, ',', '.') }}
            <br>
        ...
        @else
            ...
            @if($m->bengkel)
                <br>
                <small class="text-muted">{{ $m->bengkel }}</small>
            @endif
        @endif
    </td>
    ...
    <td>
        ...
        {{-- DOWNLOAD SURAT --}}
        @if($m->status_surat == 'approved')
            <a href="{{ route('vehicles.download-surat', [$asset, $m]) }}"
      ...
      class="d-inline">
        ...
        {{-- LIHAT NOTA --}}
        @if($m->file_nota)
            <a href="{{ asset('storage/' . $m->file_nota) }}"
               target="_blank"
        ...
        {{-- DELETE --}}
        <form action="{{ route('vehicles.delete-maintenance', [$asset, $m]) }}"
              method="POST"
              class="d-inline"
