...
class AssetPinjamanController extends Controller
{
    public function index(Request $request)
    {
        $query = Asset::where('tipe_dashboard', 'pinjaman');
        ...
        return view('assets.index-pinjaman', compact('assets', 'users'));
    }

    public function create()
    {
        $users = User::orderBy('name')->get();
        return view('assets.create-pinjaman', compact('users'));
    }
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            // tambahan info pemegang pinjaman
            'jabatan_pemegang'  => 'nullable|string',
            'alamat_pemegang'   => 'nullable|string',
            'nipp_pemegang'     => 'nullable|string',
        ]);
        ...
        // Jika aset pinjaman kategori Kendaraan, simpan detail kendaraan
        if ($validated['kategori'] === 'Kendaraan') {
            ...
            if (!empty($vehicleData)) {
                ...
                VehicleDetail::create($vehicleData);
            }
        }

        // Histori awal pinjaman
        if ($request->filled('pemegang_saat_ini')) {
            AssetHistory::create([
                ...
                'keterangan'           => 'Penerimaan awal aset pinjaman',
            ]);
        }

        return redirect()->route('assets-pinjaman.index')
            ->with('success', 'Aset Pinjaman berhasil ditambahkan!');
    }
    ...
    {
        $users = User::orderBy('name')->get();
        return view('assets.edit-pinjaman', compact('asset', 'users'));
    }
    ...
    public function update(Request $request, Asset $asset)
    {
        ...
        return redirect()->route('assets-pinjaman.index')
            ->with('success', 'Aset Pinjaman berhasil diupdate!');
    }
    ...
    public function destroy(Asset $asset)
    {
        ...
        return redirect()->route('assets-pinjaman.index')
            ->with('success', 'Aset Pinjaman berhasil dihapus!');
    }
    ...
    public function exportExcel(Request $request)
    {
        ...
        $assets   = $query->get();
        $filename = 'Data_Aset_Pinjaman_' . date('Ymd_His') . '.csv';
        $headers  = [
        ...
        $callback = function () use ($assets) {
            ...
            fputcsv($file, [
                'Kode Aset', 'Nama Aset', 'Kategori', 'Merk', 'Tipe',
                'Tahun Perolehan', 'Nilai Perolehan', 'Kondisi',
                'Lokasi', 'Pemegang Saat Ini', 'Keterangan',
            ]);
